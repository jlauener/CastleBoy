...
{
  // reset the display
  core::delayShort(5); // reset pin should be low here. let it stay low a while
  bitSet(RST_PORT, RST_BIT); // set high to come out of reset
  core::delayShort(5); // wait a while
  ...
  for (uint8_t i = 0; i < sizeof(lcdBootProgram); i++) {
    core::SPItransfer(pgm_read_byte(lcdBootProgram + i));
  }
  core::LCDDataMode();
}
...
void core::LCDDataMode()
{
  bitSet(DC_PORT, DC_BIT);
}

void core::LCDCommandMode()
{
  bitClear(DC_PORT, DC_BIT);
}

// Write to the SPI bus (MOSI pin)
void core::SPItransfer(uint8_t data)
{
  SPDR = data;
  /*
...
void core::paintScreen(uint8_t image[])
{
  ...
  for (int iy = 0; iy < (SCREEN_HEIGHT * SCREEN_BUFFER_WIDTH) / 8; iy += SCREEN_BUFFER_WIDTH)
  {
    // FIXME padding, use page address and offset instead
...
void core::sendLCDCommand(uint8_t command)
{
  LCDCommandMode();
  SPItransfer(command);
  LCDDataMode();
...
void core::allPixelsOn(bool on)
{
  sendLCDCommand(on ? OLED_ALL_PIXELS_ON : OLED_PIXELS_FROM_RAM);
}

// flip the display vertically or set to normal
void core::flipVertical(bool flipped)
{
  sendLCDCommand(flipped ? OLED_VERTICAL_FLIPPED : OLED_VERTICAL_NORMAL);
}

// flip the display horizontally or set to normal
void core::flipHorizontal(bool flipped)
{
...
void core::digitalWriteRGB(uint8_t red, uint8_t green, uint8_t blue)
{
#ifdef ARDUBOY_10
...
void core::digitalWriteRGB(uint8_t color, uint8_t val)
{
#ifdef ARDUBOY_10
  if (color == RED_LED)
  {
...
void core::delayShort(uint16_t ms)
{
